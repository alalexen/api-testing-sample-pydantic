...
def compare_json_left_in_right(json1, json2, key='', path=''):
    ...
    if isinstance(json1, dict) and isinstance(json2, dict):
        for key in json1:
            if key not in json2:
                dif_dict[key] = {"expected": json1[key], "actual": "key undefined"}
                continue
            dif_dict.update(compare_json_left_in_right(json1[key], json2[key], key, f"{path}{key}:"))
    elif json1 != json2:
         dif_dict[key] = {"expected": json1, "actual": json2, "path": path[-1]}
    return dif_dict